...
def add_ship():
    form = AddFootballTeamForm()
    if form.validate_on_submit():
        new_football_team = Football_Team(name=form.name.data, model=form.model.data)
...
def update_player2(p2):
    updated_player = db.session.query(Player).filter_by(name=p2).first()
...
def update_player3(p3):
    updated_player = db.session.query(Player).filter_by(name=p3).first()
    updated_player.name = request.form.get('name')
    updated_player.player_age = request.form.get('player_age')
...
@app.route('/update_football_team2/<name>/<manager>', methods=['GET', 'POST'])
def update_football_team2(name, manager):
    updated_football_team = db.session.query(Football_Team).filter_by(name=name, manager=manager).first()
    updated_player.name = request.form.get('name')
    updated_player.manager = request.form.get('manager')
    db.session.commit()
